...
import { FiHome, FiBookOpen, FiMessageSquare, FiBookmark, FiHelpCircle, FiUsers, FiBarChart2, FiFlag, FiMapPin, FiLogOut, FiLogIn } from 'react-icons/fi';
...
function Sidebar({ isOpen, onClose, user, onLogout }) {
  ...
  const navItems = [
    ...
    { name: 'Legal Assistant', path: '/chatbot', icon: FiMessageSquare },
      ...
      <div className={`sidebar ${isOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-nav">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} className={`sidebar-nav-item ${location.pathname === item.path ? 'active' : ''}`} onClick={onClose}>
